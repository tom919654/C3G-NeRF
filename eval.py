...
for i in tqdm(range(n_iter)):
    with torch.no_grad():
        cond = [0.]*7
        cond[random.randrange(0,7)] = 1.
        cond = torch.tensor([cond]).to(device)
        cond = cond.repeat(batch_size, 1)
        img_fake.append(model.generator(cond_data=cond, batch_size=batch_size).cpu())
...
for class_ in range(7):
    ...
    fid_score = calculate_frechet_distance(mu, sigma, fid_dict['m'], fid_dict['s'])
    out_dict['fid'] = fid_score
    print("%d FID Score (%d images): %.6f" % (class_ ,n_images, fid_score))
np.savez(out_dict_file, **out_dict)
