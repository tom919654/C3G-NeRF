...
class Generator(nn.Module):
    ...
    def __init__(self, device, z_dim=256, z_dim_bg=128, decoder=None,
        ...
        if cond:
            self.cond_encoder_1 = spectral_norm(nn.Linear(7, z_dim))
            nn.init.normal_(self.cond_encoder_1.weight, 0.0, 0.05)
            nn.init.constant_(self.cond_encoder_1.bias, 0.)
            self.cond_encoder_2 = spectral_norm(nn.Linear(7, z_dim))
            nn.init.normal_(self.cond_encoder_2.weight, 0.0, 0.05)
            nn.init.constant_(self.cond_encoder_2.bias, 0.)
    ...
    def forward(self, batch_size=32, latent_codes=None, camera_matrices=None,
        ...
        if self.cond and cond_data is not None:
            latent_codes = (latent_codes[0]*(self.cond_encoder_1(cond_data)).view([batch_size, 1, self.z_dim]).expand_as(latent_codes[0]),
                            latent_codes[1]*(self.cond_encoder_2(cond_data)).view([batch_size, 1, self.z_dim]).expand_as(latent_codes[1]),
                            latent_codes[2],
                            latent_codes[3]
                            )
        ...
        if return_alpha_map:
            rgb_v, alpha_map = self.volume_render_image(
                ...
                not_render_background=not_render_background, cond_data=cond_data)
            return alpha_map
        else:
            rgb_v = self.volume_render_image(
                latent_codes, camera_matrices, transformations, bg_rotation,
                mode=mode, it=it, not_render_background=not_render_background,
                only_render_background=only_render_background, cond_data=cond_data)
    ...
    def volume_render_image(self, latent_codes, camera_matrices,
        ...
        for i in range(n_iter):
            if i < n_boxes:  # Object
                ...
                feat_i, sigma_i = self.decoder(p_i, r_i, z_shape_i, z_app_i, cond_data=cond_data)
