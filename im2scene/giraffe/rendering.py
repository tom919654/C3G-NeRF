...
class Renderer(object):
    ...
    def render_full_visualization(self, img_out_path,
        ...
        for rp in render_program:
            ...
            if rp == 'image_translations':
                self.set_random_seed()
                self.image_translations(img_out_path)

    def image_translations(self, img_out_path, batch_size=15, n_steps=32):
        gen = self.generator
        ...
        for step in range(n_steps):
            with torch.no_grad():
                i = 1.0 - (step*1.0)/(n_steps-1)
                self.cond_data[0][0] = i
                self.cond_data[0][1] = 1.0 - i
                out_i = gen(batch_size, latent_codes, camera_matrices,
                            transformations, bg_rotation, cond_data=self.cond_data.repeat(batch_size, 1), mode='val')
            out.append(out_i.cpu())
        out = torch.stack(out)
        out_folder = join(img_out_path, 'image_translations')
        makedirs(out_folder, exist_ok=True)
        self.save_video_and_images(
            out, out_folder, name='image_translations',
            add_reverse=True)
    ...
    def render_object_rotation(self, img_out_path, batch_size=15, n_steps=32):
        ...
        for step in range(n_steps):
            ...
            with torch.no_grad():
                out_i = gen(batch_size, latent_codes, camera_matrices,
                            transformations, bg_rotation, cond_data=self.cond_data.repeat(batch_size, 1) + torch.randn(batch_size, 7).to(self.device)*0.05, mode='val')
            out.append(out_i.cpu())
        out = torch.stack(out)
    ...
    def render_camera_elevation(self, img_out_path, batch_size=15, n_steps=32):
        ...
        for step in range(n_steps):
            ...
            with torch.no_grad():
                out_i = gen(
                    batch_size, latent_codes, camera_matrices, transformations, bg_rotation, cond_data=self.cond_data.repeat(batch_size, 1) + torch.randn(batch_size, 7).to(self.device)*0.05,
                    mode='val')
            out.append(out_i.cpu())
        out = torch.stack(out)
        ...
        for i in range(1, 7):
            ...
            with torch.no_grad():
                out = gen(
                    ...
                    bg_rotation, cond_data=self.cond_data.repeat(batch_size, 1) + torch.randn(batch_size, 7).to(self.device)*0.05, mode='val').cpu()
            outs.append(out)
