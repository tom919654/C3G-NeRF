...
class Trainer(BaseTrainer):
    ...
    def __init__(self, model, optimizer, optimizer_d, device=None,
                 ...
                 overwrite_visualization=True, cond=True, **kwargs):
    ...
    def train_step_generator(self, data, it=None, z=None):
        ...
        cond_data = data.get('cond').to(self.device)

        if self.multi_gpu:
            latents = generator.module.get_vis_dict()
            if self.cond:
                x_fake = generator(cond_data = cond_data, **latents)
            else:
                x_fake = generator(**latents)
        else:
            if self.cond:
                x_fake = generator(cond_data = cond_data)
            else:
                x_fake = generator()

        d_fake = discriminator(x_fake, cond_data = cond_data)
    ...
    def train_step_discriminator(self, data, it=None, z=None):
        ...
        if self.cond:
            d_real = discriminator(x_real, cond_data=cond_data)
        else:
        ...
        reg = 1. * compute_grad2(d_real, x_real).mean()
        loss_d_full += reg
